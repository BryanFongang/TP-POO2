...
public class Phone extends Object {
    public Phone(Integer codeBarre, LocalDate dateFabrication, String marque, String model, String stockage, String ram) {
        super(codeBarre, dateFabrication, marque, model, stockage, ram);
    }

    // Constructor avec tous les attributs
    public Phone(Integer codeBarre, LocalDate dateFabrication, String marque, String model,
    ...
    @Override
    public Phone rechercherDansBD(Connection conn, Integer codeBarre) throws SQLException {
        String query = "SELECT * FROM Phone WHERE codeBarre = ?";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            if (rs.next()) {
                ...
                return new Phone(codeBarre, dateFabrication, marque, model, stockage, ram,
                        isStolen, dateVol, proprietaire, contactProprietaire);
            }
        }

        return null; // Aucun téléphone trouvé avec ce code-barres
    }
    ...
    public boolean signalerVol(Connection conn, LocalDate dateVol, String proprietaire, String contactProprietaire) throws SQLException {
        String query = "UPDATE Phone SET isStolen = ?, dateVol = ?, proprietaire = ?, contactProprietaire = ? WHERE codeBarre = ?";
    ...
    public boolean enregistrerDansBD(Connection conn) throws SQLException {
        String query = "INSERT INTO Phone (codeBarre, dateFabrication, marque, model, stockage, ram, isStolen, dateVol, proprietaire, contactProprietaire) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
